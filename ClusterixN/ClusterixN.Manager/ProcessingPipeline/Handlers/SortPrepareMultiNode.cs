...
{
    internal class SortPrepareMultiNode : HandlerBase
    {
        public SortPrepareMultiNode(IServerCommunicator server, IQueryManager queryManager, QueryBufferManager queryBufferManager,
        ...
        private void SendNewQueryToSortNode(Node node)
        {
            ...
            if (query != null)
            {
                if (query.SortQuery == null)
                {
                    ...
                    throw new NotImplementedException("СОРТ ДОЛЖЕН БЫТЬ!!!!!");
                }

                if (query.SortQuery.Status != QueryStatus.Wait) return; //уже обработанао

                node.QueryInProgress++;
                var sortQuery = query.SortQuery;

                QueryManager.SetSubQueryStatus(query.SortQuery.QueryId, QueryStatus.TransferToSort);
