...
namespace ClusterixN.Manager.Managers
{
    ...
    public class SortOnJoinQuerySourceManager : QuerySourceManagerBase
    {
        #region Queries
...
WHERE
	...
	L_LINESTATUS",
                ...
                1,
                builder.CreateRelation(select1, "L",
                    builder.CreateRelationSchema(
                        new List<Field>()
                        {
                            new Field() {Name = "L_RETURNFLAG", Params = "CHAR(1) NOT NULL"},
                        ...
                        new List<Index>()
                        {
                            new Index() {Name = "L_RETURNFLAG_LINESTATUS", FieldNames = new List<string>() { "L_RETURNFLAG", "L_LINESTATUS" } },
                        })
                ),
                builder.CreateEmptyRelation(),
                @"SELECT
    L_RETURNFLAG,
	L_LINESTATUS,
	SUM_QTY,
	...
	" + Constants.RelationNameTag + @";",
                builder.CreateRelation(selectL, "L",
                    builder.CreateRelationSchema(
                        new List<Field>()
                        {
                            new Field() {Name = "REVENUE", Params = " DECIMAL(18,4) NOT NULL"},
                        },
                        new List<Index>()
                        {
                        })
...
FROM
                ...
                builder.CreateRelationSchema(
                    ...
                    new List<Index>()
                    {
                        new Index() {Name = "P_NATION_O_YEAR", FieldNames = { "NATION", "O_YEAR"}}
                    })
                ,
                5,
                builder.CreateRelation(join4),
...
FROM
                                    " + Constants.LeftRelationNameTag + @" AS PROFIT
GROUP BY
	NATION,
	O_YEAR",
                builder.CreateRelationSchema(
...
FROM
	" + Constants.RelationNameTag + @";",
                builder.CreateRelation(join6, QueryRelationStatus.Wait)));
...
FROM
                ...
                builder.CreateRelationSchema(
                    ...
                    new List<Index>()
                    {
                        new Index() {Name = "sort10", FieldNames = new List<string>() { "C_CUSTKEY", "C_NAME", "C_ACCTBAL", "C_PHONE", "N_NAME", "C_ADDRESS", "C_COMMENT"} },
                    }),
                3,
                builder.CreateRelation(join2),
                builder.CreateRelation(selectN, "N",
...
FROM
	" + Constants.RelationNameTag + @";",
                builder.CreateRelation(join4, QueryRelationStatus.Wait)));
...
GROUP BY
	C_CUSTKEY",
                builder.CreateRelationSchema(
                    ...
                    {
                        new Field() {Name = "C_CUSTKEY", Params = " INT NOT NULL"},
                        new Field() {Name = "C_COUNT", Params = " INT NULL"},
                    },
                    new List<Index>()
                    {
                    })
...
FROM
	...
	" + Constants.RelationNameTag + @" AS SORT
GROUP BY
	C_COUNT
ORDER BY
	CUSTDIST DESC,
