...
{
    class SubqueryTableItemListener : MySqlParserBaseListener
    {
        ...
        public override void EnterQueryExpression(MySqlParser.QueryExpressionContext context)
        {
            if (_isOtherListener == 1)
            {
                ...
                QueryExpression queryExpression =
                    new QueryExpression(context.SourceInterval, context, context.GetText());

                Rules.Add(queryExpression);

            }
            _isOtherListener++;
        }

        public override void ExitQueryExpression(MySqlParser.QueryExpressionContext context)
        {
            _isOtherListener--;
